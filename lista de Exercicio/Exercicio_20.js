...
● Um número inteiro N (entre 0 e 11), representando uma coluna específica de
...
● Identifique os elementos da área verde da matriz (correspondente à coluna
N) e, conforme a operação indicada ('S' ou 'M'), calcule e exiba o resultado.
 */
...
console.log("\nEscolha a coluna da Matriz entre [0 e 11]:")  
...
if(num >= 0 && num <= 11){  
    do{  
        ...
        if(operacao === "s"){  
            let soma = 0;  
            for(let linha = 0; linha < matrizAleat.length; linha++){  
                soma += matrizAleat[linha][num]  
            }  
            console.log(`\nA soma de todos os elementos da coluna ${num} é: ${soma}`);  
            ...
            let media = soma / matrizAleat.length;  
            console.log(`\nA média de todos os elementos da coluna ${num} é: ${media.toFixed(2)}`);  
        ...
        let continuar = leia.question('').toLowerCase();  
        if(continuar !== 's'){  
            break;  
        }  
